...
class DeadlockDetector:
    ...
    def resolve(self, cycle: List[int], robots: Dict[int, "Robot"],
        ...
        # Tie-break: longest wait time first
        sacrifice_id = max(
            cycle,
...
class PathPlanner:
    ...
    def plan(self, start: int, goal: int,
        ...
        Args:
            blocked_edges: edges to treat as impassable (for rerouting)
            banned_next_node: node to avoid as the immediate next step
        ...
        while open_heap:
            ...
            for nbr in self.graph.successors(cur):
                ...
                g_new    = best_g[cur] + base + cong_pen + safe_pen
...
class TrafficController:
    ...
    def _compute_bid(self, robot: Robot, tick: int) -> float:
        ...
        return robot.priority_weight * (1.0 / est_wait) * urgency
    ...
    def build_wait_graph(self, robots: Dict[int, Robot]) -> Dict[int, Optional[int]]:
        ...
        for rid, r in robots.items():
            if r.state in (RobotState.WAITING, RobotState.MOVING):
...
class Simulation:
    ...
    def _do_planning(self, rob: Robot):
        ...
        path = self.planner.plan(rob.current_node, rob.goal, banned_next_node=banned)
        ...
        if not self.controller.ghost_predict_clear(rob, self.robots, path, 0):
            blocked = {(path[i], path[i + 1]) for i in range(len(path) - 1)}
            alt = self.planner.plan(rob.current_node, rob.goal, blocked, banned_next_node=banned)
            if alt and len(alt) > 1:
                path = alt
                logging.info(f"R{rob.id}: rerouted (ghost conflict)")
        ...
        if worst_cong > CONGESTION_THRESHOLD:
            ...
            alt = self.planner.plan(rob.current_node, rob.goal, bad, banned_next_node=banned)
            if alt:
                path = alt
        ...
        rob.next_node     = path[1] if len(path) > 1 else -1
    ...
    def _do_moving(self, rob: Robot):
        ...
        if not self.controller.is_edge_free(edge, rob.id):
            occ = self.controller.edge_occ.get(edge)
            self._enter_wait(rob, blocker=occ)
            return
        ...
        if self.graph.graph.has_edge(*rev):
            if not self.controller.is_edge_free(rev, rob.id):
                occ = self.controller.edge_occ.get(rev)
                self._enter_wait(rob, blocker=occ)
                return
        ...
        if self.controller.check_emergency_stop(rob, self.robots, self.tick):
        ...
        # Register on edge at start of traversal
    ...
    def _handle_deadlocks(self):
        ...
        for cycle in cycles:
            ...
            # Banned next node logic instead of teleport backtrack
            if rob.path_index < len(rob.path) - 1:
                rob.banned_next_node = rob.path[rob.path_index + 1]
                logging.info(f"R{sid}: deadlock sacrifice. Banning node "
                             f"{self.graph.node_labels[rob.banned_next_node]} for replan")
